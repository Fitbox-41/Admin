...
const Refunds = () => {
  ...
  const fetchRefunds = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Show all cancelled orders that were paid
  ...
  const handleRefundDone = async (orderId) => {
    ...
    try {
      ...
      if (res.ok) {
        // Update in list
        setOrders(orders.map(o => o._id === orderId ? { ...o, refunded: 'yes', isRefunded: true } : o));
      } else {
  ...
  const pendingRefunds = filteredOrders.filter(o => o.refunded !== 'yes' && o.isRefunded !== true);
  const refundedOrders = filteredOrders.filter(o => o.refunded === 'yes' || o.isRefunded === true);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-card overflow-hidden mb-8">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-text">Pending Refunds</h2>
          <span className="bg-primary/10 text-primary px-3 py-1 rounded-full text-xs font-medium">
            {pendingRefunds.length} orders
          </span>
        </div>
        {loading ? (
      ...
      <div className="bg-white rounded-2xl shadow-card overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-text">Order Refunded</h2>
          <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-medium">
            {refundedOrders.length} processed
          </span>
        </div>
        {loading ? (
          <div className="flex justify-center items-center h-64">
          ...
          <div className="flex flex-col items-center justify-center py-16 text-text-light">
            <p className="text-sm">No refunded orders found.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-gray-100">
                {refundedOrders.map(order => (
                  <tr key={order._id} className="bg-green-50/50 hover:bg-green-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-medium text-text">
                        {order.invoiceNumber ? order.invoiceNumber : order._id.slice(-8).toUpperCase()}
                      </div>
